\documentclass[margin]{res}
   
	\usepackage[bookmarks=true]{hyperref}
	\usepackage[open]{bookmark}
    \usepackage{xcolor}
    
% \topmargin=-0.8mm

\begin{document}

\moveleft\hoffset\vbox{\LARGE\bf Mohammad Ali Arabyazdi}
\moveleft\hoffset\vbox{\hrule width\resumewidth height 1pt}\smallskip
\moveleft\hoffset\vbox{Address: Saberi St., North of Sheikh-Bahaei St., Tehran, Iran}
\moveleft\hoffset\vbox{Email: \href{mailto:ma.yazdi75@gmail.com}{\nolinkurl{ma.yazdi75@gmail.com}} and \href{mailto:ma.yazdi75@aut.ac.ir}{\nolinkurl{ma.yazdi75@aut.ac.ir}}}
\moveleft\hoffset\vbox{Home-Page: \url{https://aliyazdi75.github.io}}
\moveleft\hoffset\vbox{Cell Phone: +98 912 037 1375}

\begin{resume}

%----------------------------------------------------------------------------------------
% EDUCATION SECTION
%----------------------------------------------------------------------------------------
	        
\vspace{-5mm}

\section{EDUCATION}

\begin{itemize} \itemsep -5pt

\item \textbf{\href{http://aut.ac.ir/www/aut/main/}{Amirkabir University of Technology}} (Tehran Polytechnic), Tehran, Iran\\
B.Sc., Computer Engineering, Software \\ \textit{September 2015 - Peresent}\\
GPA: \textbf{3} / 4
\hspace{11px} \\

\item \textbf{\href{http://adab.sch.ir/HSchool}{Adab High School}}, Tehran, Iran\\
Diploma in Mathematics and Physics \\ \textit{September 2011 - June 2015}\\
GPA: High School \textbf{3.9} / 4

\end{itemize}

%----------------------------------------------------------------------------------------
% TECHNICAL SKILLS SECTION
%----------------------------------------------------------------------------------------

\vspace{-3mm}

\section{TECHNICAL SKILLS}
	
\begin{itemize} \itemsep -8pt

\item \textbf{Programming Languages:}\\
Java, C/C++, Python, Shell Scripting, Assembly                                                      \\

\item \textbf{Database Systems:}\\
Windows, Linux (Ubuntu)                                                                             \\

\item \textbf{Operating System:}\\
MySQL, SQL Server                                                                                   \\

\item \textbf{Web Development:}\\
HTML (XHTML and HTML 5), CSS, JavaScript, Django, PHP, Symfony, jQuery                              \\

\item \textbf{Other:}\\
Android Programming, Microcontroller Programming, Proteus, Cisco Packet Tracer, NMap, VHDL, Verilog \\
	
\end{itemize} 

%----------------------------------------------------------------------------------------
% TOP ACADEMIC COURSE PROJECTS SECTION
%----------------------------------------------------------------------------------------

\vspace{-6mm}

\section{TOP ACADEMIC COURSE PROJECTS}

\begin{itemize} \itemsep +3pt

\item \textbf{Internet Engineering}, {\sl php, symfony, xampp}\\
\textit{\href{https://github.com/aliyazdi75/sms-manager}{SMS-Manager}: An api platform to send and manage SMS}

\item \textbf{Computer Networks}, {\sl Python}\\
\textit{\href{https://github.com/aliyazdi75/DNS-UDP-TCP-Proxy}{DNS-UDP-TCP-Proxy}: A python proxy that receive HTTP packets on UDP then sends them on TCP with DNS server}

\item \textbf{Information \& Communication Security}, {\sl Python}\\
\textit{\href{https://github.com/aliyazdi75/My-Security-Course}{My-Security-Course}: Course project using MD5, SHA256, DES encryption and work with cryptool software nmap and using ms08\_067 exploit and make trojan}

\item \textbf{Microprocessor \& Assembly Language}, {\sl C, Proteus}\\
\textit{\href{https://github.com/aliyazdi75/Digital-Watch-with-LCD}{Digital-Watch-with-LCD}: A digital watch with lcd and alarm using microcontroller ATMEGA16}

% \item \textbf{Theory of Machines and Languages}, {\sl Java}\\
% \textit{\href{https://github.com/aliyazdi75/Simplifying-CFG}{Simplifying-CFG}: Course project to simplifying context free grammar}

\item \textbf{Computer Architecture}, {\sl VHDL}\\
\textit{\href{https://github.com/aliyazdi75/Basic-Computer-SAYEH-vhdl}{Basic-Computer}: Design and implementation of a small modular processor}

\item \textbf{Data Structure \& Algorithms}, {\sl Java}\\
\textit{\href{https://github.com/aliyazdi75/Finite-State-Machine}{Finite-State-Machine}: Course project using 4 type of data structure: AVL tree, balance TST, trie tree and hash}

\item \textbf{dvanced Computer Programming}, {\sl Java}\\
\textit{\href{https://github.com/aliyazdi75/Planet\_vs\_Zombies}{Planet-vs-Zombies}: An OOP game}

\end{itemize}

\moveleft.5\hoffset\centerline{\textit{}}
\moveleft.5\hoffset\centerline{\textit{Last Update: March 2019}}

\end{resume}
\end{document}